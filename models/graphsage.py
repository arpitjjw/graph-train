...
class GNNStack(torch.nn.Module):
    ...
    def encode(self,data,nodes): 
    
    
        idx=list(nodes.cpu().detach().numpy())
        central_nodes,edges,mapping,mask=k_hop_subgraph(node_idx=list(nodes.cpu().detach().numpy()),edge_index=data.edge_index,num_hops=self.params['num_layers'],relabel_nodes=True)
        x, edge_index,score = data.x[central_nodes], edges, data.score[mask]
        self.map_id_index=dict(zip(idx, list(mapping.cpu().detach().numpy())))
    ...
    def contrastive_loss(self,z,edges,score,alpha=0.2, mode='cos'):
        
        score_ = self.cos(z[[edges[0].cpu().apply_(lambda x:self.map_id_index[x])]] , z[[edges[1].cpu().apply_(lambda x:self.map_id_index[x])]]) 
